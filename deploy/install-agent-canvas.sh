#!/usr/bin/env bash
# Install OpenHands Agent Canvas (npm + uv). Used on control plane and runners.
# Usage: install-agent-canvas.sh [/path/to/env-file]
set -euo pipefail
...
echo "[install-agent-canvas] $(agent-canvas --version)"
