...
2. Configure Canvas **Manage Backends** with `/usr/local/bin/copilot` + args `acp` instead of Cursor.
3. Re-run bootstrap after fixing secrets — failures are no longer ignored.
